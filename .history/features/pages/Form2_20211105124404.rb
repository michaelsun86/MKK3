...
def Province_Area(name)
    provinces = $driver.find_element(:xpath, '//div[@class="wizard-navigation"]/div[2]/div[2]/div/fieldset[8]/div/select/option')
    provinces.each { |province| province.click if province.text == name }
    sleep(2)
end    

def City_Area(name)
    cities = $driver.find_element(:xpath, '//div[@class="wizard-navigation"]/div[2]/div[2]/div/fieldset[9]/div/select/option')
    cities.each { |city| city.click if city.text == name }
    sleep(2)
end    

def Districts_Area(name)
    districts = $driver.find_element(:xpath, '//div[@class="wizard-navigation"]/div[2]/div[2]/div/fieldset[10]/div/select/option')
    districts.each { |district| district.click if district.text == name }
    sleep(2)
end    

def Subdistricts_Area(name)
    subdistricts = $driver.find_element(:xpath, '//div[@class="wizard-navigation"]/div[2]/div[2]/div/fieldset[11]/div/select/option[2]')
