...
def Last_Education(name)
    ...
    sleep(2)
    educations = $driver.find_elements(:xpath, '//div[@class="wizard-tab-content"]/div[1]/div/fieldset[7]/div/select/option').value
    for i in educations do
        education = $driver.find_element(:xpath, '//div[@class="wizard-tab-content"]/div[1]/div/fieldset[7]/div/select/option')
        edu_name = education.text
        if edu_name == name
            education.click
        end
    end
...
def Residence_Status()
    ...
    residences.each do |residence|
        residence.text
        if residence.text == "Milik Sendiri"
            residence.click
            break
        end  
    end        
    #for i in residences do
        #residence = $driver.find_element(:xpath, '//div[@class="wizard-tab-content"]/div[1]/div/fieldset[8]/div/select/option')
        #res_name = residence.text
        #if res_name == "Milik Sendiri"
            #residence.click
        #end
    #end
    sleep(5)
